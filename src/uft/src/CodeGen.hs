...
makeIf toX x e1 e2 = do
  ...
  branch1 <- toX e1
  return $
    s (U.ifgoto x l) <> branch2 <> s (U.goto l')
      <> s (U.deflabel l)
      <> branch1
      <> s (U.deflabel l')
 ...
 where
  (table, cases) `f` ((cons, arity), e) =
    do
      ...
      body <- gen e
      return
        ( (conLiteral (cons, arity), arity, l) : table
...
toReg' dest e = case e of
  ...
  K.VMOP{} -> mconcat <$> sequence [forEffect' e, return (s $ U.reglit "loadliteral" dest O.Nil)] -- "undefined behavior"
  ...
  K.While x e1 e2 -> mconcat <$> sequence [forEffect' (K.While x e1 e2), toReg' dest (K.Literal $ O.Bool False)]
  -- floatable forms
  K.Let x e1 e' -> mconcat <$> sequence [toReg' x e1, toReg' dest e']
  K.Seq e1 e2 -> mconcat <$> sequence [forEffect' e1, toReg' dest e2]
  K.Assign x e -> mconcat <$> sequence [toReg' x e, toReg' dest (K.Name x)]
  K.Captured i -> return $ s $ U.captured dest i
  K.ClosureX (K.Closure args body captured) -> do
    b' <- toReturn' body
    return $
      s (A.LoadFunc dest (length args) (b' []))
  ...
  K.Block (r : rs) ->
    return $
      s (U.mkblock dest r (length (r : rs)))
...
forEffect' e = case e of
  ...
  K.VMOPGLO (P.HasEffect (P.Base op _)) args v ->
    return $
      s $ A.ObjectCode (O.RegLit op (head args) v)
  ...
  K.While x e e' -> do
    ...
    check <- toReg' x e
    return $
      s (U.goto l) <> s (U.deflabel l') <> body
        <> s (U.deflabel l)
        <> check
        <> s (U.ifgoto x l')
  K.Let x e e' -> mconcat <$> sequence [toReg' x e, forEffect' e']
  K.Seq e1 e2 -> mconcat <$> sequence [forEffect' e1, forEffect' e2]
...
toReturn' e =
  let returnx x = U.regs "return" [x]
      returnIn0 e = mconcat <$> sequence [toReg' 0 e, return (s $ returnx 0)]
   in case e of
        ...
        K.If x e1 e2 -> do
          ...
          branch1 <- toReturn' e1
          return $
            s (U.ifgoto x l) <> branch2
              <> s (U.deflabel l)
              <> branch1
        K.While x e e' -> returnIn0 e
        -- floatable
        K.Seq e1 e2 -> mconcat <$> sequence [forEffect' e1, toReturn' e2]
        K.Let x e e' -> mconcat <$> sequence [toReg' x e, toReturn' e']
        ...
        K.LetRec bindings body -> do
          ...
          mconcat
            <$> sequence
              [ letrec toReturn' bindings body
              , return (body' <> s (returnx 0))
              ]
        K.Block{} -> returnIn0 e
...
letrec gen bindings body =
  let alloc (f_i, K.Closure formals body captures) =
        mconcat <$> sequence [toReg' f_i (K.FunCode formals body), return (s (U.mkclosure f_i f_i (length captures)))]
      init (f_i, K.Closure formals body captures) = l (mapi (U.setclslot f_i) captures)
   in mconcat <$> sequence [foldr (\b l -> (<>) <$> (alloc b) <*> l) (return empty) bindings, return (mconcat (map init bindings)), gen body]
